...
const Sidemenu = () => {
  ...
  const { adminLogout } = useContext(HandleContext);
  return (
    <>
      ...
      >
        ...
        <Link to={"/"}>
          {" "}
          <img className="relative left-10" src={logo} alt="logo" />
        </Link>
        <div className="w-full flex flex-col items-center h-full justify-between">
          <nav className="w-full flex flex-col items-start justify-center  gap-7">
            <Link
              className={`flex flex-row items-center justify-start px-5 gap-2 text-[20px] w-full py-3 ${
                location.pathname === "/" ? "bg-[#FE6903]" : "bg-transparent"
            ...
            <Link
              ...
              to={"/users"}
            >
              <FaUser /> <span className="capitalize">Users</span>
            </Link>
          </nav>
          <button
            onClick={adminLogout}
